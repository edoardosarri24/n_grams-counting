...
In this section we describe the idea that was used in the parallel implementation. We will treat the pipeline of the program in Section \ref{sec:par_pipeline}.
...
To address this efficiently, we implemented a thread-local arena: at the beginning of the population phase, each thread creates a temporary private arena; the allocation of memory to create a new node doesn't touch the global arena, but only the private arena and this means no synchronization.
...
The dimension of blocks in the local arena is 1MB. This minimizes the frequency of \texttt{malloc} calls, which implies a lock in the system and so an overhead.
...
The extraction of statistics is the same as the sequential version (see Section~\ref{sec:sequential_statistics}) and it is performed sequentially by the main thread after the parallel region has completed. The execution time of this step is irrelevant compared to the other parts.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Pipeline}
\label{sec:par_pipeline}
The pipeline of the map-reduce approach implementation is composed of three steps: initialization of the variables, population (Map) of the local hash table, and merging (Reduce) the local hash table into the global one. The code of this part is mainly in the \texttt{populate\_hashtable} function in the \href{https://github.com/edoardosarri24/parallel-trigrams/blob/ef94c1127b974ed4241ad355c5d9b218b3484e1f/parallel/src/main_functions.c}{main\_functions.c} file.
...
\begin{itemize}
    ...
    \item \textbf{Local Tables Array}: We allocate an array of pointers to HashTable structs of size \texttt{omp\_get\_max\_threads()}. This function returns the number of threads that OpenMP will allocate in a parallel region; if you use the classic function like \texttt{omp\_get\_num\_threads()} out of a parallel region, the function returns 1.
\end{itemize}

\myskip

We enforced the variable scope passing to the OpenMP clause \texttt{default(none)} on the parallel region directive. This avoids accidental race conditions or unintended sharing. More in detail we have defined as:
\begin{itemize}
...
The parallel region isn't created with a simple \texttt{\#pragma omp parallel for} on the mapped file buffer. The reason is that the hand made chunking is more useful and powerful in this case, leaving more flexibility to us to define the $n$-gram boundaries.
...
\begin{itemize}
    ...
    \item \textbf{Zero-Copy Node Relinking}: When merging a local bucket into a global one, if an n-gram is found we sum the counters, if it's not found in the global table we simply relink the existing node pointer from the local table to the global table; this avoids several calls to \texttt{malloc}. At the end of the parallel region, we use the \texttt{arena\_connect} function to append the local memory arenas (containing the relinked nodes) to the global one.
    \item \textbf{Dynamic Scheduling}: We use a dynamic assignment of the partition to the thread due to the sparsity of the hash table. In this way if a thread finishes earlier with respect to the others, all cores remain busy.
    \item \textbf{False Sharing}: With the usage of \texttt{dynamic} we could have a potential performance killer with false sharing: since multiple pointers (8 bytes each) fit in a single cache line (typically 64 bytes), adjacent buckets share the same cache line; by default we have $\texttt{block\_size}=1$, so if two threads process adjacent buckets, they would continuously invalidate each other's cache lines. So we have defined the block size as $(64/\texttt{sizeof(Node*)})\times16$: each chunk is aligned with cache lines; the chunk is large enough (16 cache lines, 1024 bytes) to amortize the OpenMP scheduling overhead. This empirical dimension must be larger if during the profiling we see that the program spends too much time in \texttt{malloc} function.
    \item \textbf{Implicit Barrier}: At the end of a \texttt{\#pragma omp for} there is an implicit barrier. Inserting a \texttt{nowait} clause would be a mistake, because it implies that the local table of the other thread will be freed causing a segmentation fault.
\end{itemize}